...
public:
	...
	void activate()
	{
		...
		action_group->add(
				Gtk::Action::create("template", _("_Template")));
	...
	void add_ui_from_file(const guint number, const Glib::ustring &filename)
	{
		...
		Glib::ustring action_accel = "";//(number < 10) ? Glib::ustring::compose("<alt>%1", (number+1) % 10) : "";

		action_group_templates->add(
				Gtk::Action::create(action_name, label), Gtk::AccelKey(action_accel),
				sigc::bind(
					sigc::mem_fun(*this, &TemplatePlugin::on_template_activate), fullname, charset));
	...
	void on_save_as_template()
	{
		...
		Glib::ustring uri = Glib::filename_to_uri(
				Glib::build_filename(
					...
					Glib::ustring::compose("[%1][%2]", newdoc->getName(), newdoc->getCharset())));
